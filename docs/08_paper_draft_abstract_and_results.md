...
In this ablation, we patched activations from the correct trace of an entirely *different* math problem into the committed-wrong trace. The flip rate collapsed to **23.1%** (3/13). The resulting **31.4 percentage-point gap** (representing a 2.4× effect ratio) exceeds our pre-registered design threshold and provides strong causal evidence. The high steering efficacy in Block A is fundamentally driven by problem-specific semantic content embedded in the latent states, not by generic residual-stream perturbation.

### 4.3 The Functional Rift: Layer-Wise Commitment

To map the precise depth at which the model commits to its terminal answer, we performed a mechanistic sweep across the 48 transformer layers of Qwen2.5-14B-Instruct (Blocks C and D). The resulting layer-wise flip rates reveal a distinct tripartite architecture of latent reasoning, which we term the **Functional Rift**:
...
3. **The Cliff (Layers 44–47):** At the final layers of the model, the flip rate hits a hard **0.0%** (0/11). Assuming a latent baseline rate of 30%, observing exactly zero flips across 11 independent trials is highly unlikely ($p \approx 0.02$). This statistically significant zero demonstrates **causal locking**. Even when injected with perfectly correct mathematical reasoning, the late layers ignore the new representations and rigidly project the established in-context prior into the original incorrect vocabulary output.
